...
def postprocessing(predi, nc=9, process='train'):  # Add nc as an argument to make it flexible
    # Check input type and unpack if necessary
    if isinstance(predi, tuple):
        x, _ = predi  # Tuple: (processed predictions, raw feature maps)

        # Debugging tuple structure
        # print(f"predi is a tuple with length: {len(predi)}")
        # print(f"Type of x: {type(x)}")
        if isinstance(x, list):
            # print(f"[VAL] x shapes: {[layer.shape for layer in x]}")
            pass  # Keep as is to handle the list case
        else:
            # print(f"x is not a list, but {type(x)}")
            # if isinstance(x, torch.Tensor):
            #     print(f"x shape: {x.shape}")
            # elif isinstance(x, tuple):
            #     print(f"x tuple content shapes: {[t.shape if isinstance(t, torch.Tensor) else type(t) for t in x]}")
            # elif x is None:
            #     print("x is None")
            # else:
            #     print(f"x unexpected content: {x}")
            raise ValueError(f"Expected list for x in val mode, but got {type(x)}")
    elif isinstance(predi, list):
        x = predi
        # z = None
        # print(f"[TRAIN] x shapes: {[layer.shape for layer in x]}")
    else:
        raise ValueError(f"Unexpected predi type: {type(predi)}")

    # Proceed with x as list
    grid = [torch.empty(0) for _ in range(len(x))]
    z = []
    anchor_grid = [torch.empty(0) for _ in range(len(x))]
    ...
    anchors = torch.tensor(anchors).float().view(len(x), -1, 2)
    # anchors[0] = anchors[0] / stride[0]
    # anchors[1] = anchors[1] / stride[1]
    # anchors[2] = anchors[2] / stride[2]
    no = nc + 5  # number of outputs per anchor (nc + 5)

    # print(f"len(x): {len(x)}")
    # print(f"x shapes: {[layer.shape for layer in x]}")
    # print(f"Original anchors shape: {anchors.shape}")

    for i in range(len(x)):
        bs, _, ny, nx = x[i].shape  # x(bs,255,20,20) to x(bs,3,20,20,85)
        x[i] = x[i].view(bs, 3, no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()  # Dynamically set based on nc

        if grid[i].shape[2:4] != x[i].shape[2:4]:
            grid[i], anchor_grid[i] = make_grid(nx, ny, i, anchors, stride)

        xy, wh, conf = x[i].sigmoid().split((2, 2, nc + 1), 4)  # Dynamically split based on nc
        xy = (xy * 2 + grid[i]) * stride[i]
        wh = (wh * 2) ** 2 * anchor_grid[i]
        y = torch.cat((xy, wh, conf), 4)
        # print(f"Layer {i} - y shape: {y.shape}, expected shape for view: {(bs, (len(anchors[0]) // 2) * nx * ny, no)}")
        z.append(y.view(bs, 3 * nx * ny, no))  # Append without concatenating, 3 = na = len(anchors[0]) // 2

    # Return z as a list of layer outputs
    # print("postprocessing output shapes by layer:", [layer.shape for layer in z])
    return x if process=='train' else (torch.cat(z, 1),) if process=='export' else (torch.cat(z, 1), x)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                predi = model(imgs)
                # print(f"predi type: {type(predi)}")
                pred = postprocessing(predi, process='train')
                # Print postprocessing output for debugging
                # print("Adjusted postprocessing final output shape:", pred[0].shape)  # Shape expected by loss function
